...
class ToDoListController: UITableViewController {
    ...
    let storage = AppDelegate.shared.storage
    private var items: StorageResults<ToDoItem>?
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
		items = ToDoItem.all(in: storage).sorted(by: \.isCompleted)
    }
    ...
    @IBAction func addItem() {
        userInputAlert("Add Todo Item") {[unowned self] text in
			ToDoItem.add(text: text, in: self.storage)
        }
    }
    
    func toggleItem(_ item:inout ToDoItem) {
		item.toggleCompleted(in: storage)
    }
